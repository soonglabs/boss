...
var DEFAULT_IMAGE = new Dir('root', 'root', null);
        ...
        bin.files['logout'] = new File('root', "(function(args, client){\nboss.lib.pop();\n})", {
                    description : 'logout'
                });
        ...
        bin.files['echo'] = new File('root', "(function(args, client){boss.lib.print.log(args[1], client)})", {
             description : 'echo string to console'
        });

        bin.files['inspect'] = new File('root', "(function(args, client){console.log(boss)})", {
             description : 'log boss object for inspection'
        });

        bin.files['test'] = new File('root', "(function(args, client){boss.lib.print.log('test ls', client);boss.cmd.ls(args, client);})", {
        ...
        lib.files['print'] = new File('root', "({\n"+
            ...
            "}\n"+
        "})");
        lib.files['set_prompt'] = new File('root', "(function(txt, client){client.set_prompt(txt)})");
        lib.files['push'] = new File('root', "(function(fn){boss.interpreters.push(fn)})");
        lib.files['pop'] = new File('root', "(function(){boss.interpreters.pop()})");
        ...
        lib.files['Login'] = new File('root', 
         ...
         lib.files['LoadFS'] = new File('root', 
            "(function(boss){\n"+
                "//choose fs to load\n"+
            "})"
        );
...
var JSON_IMAGE = {"name":"root","owner":"root","parent":null,"created":1467054557216,"dirs":{"bin":{"name":"bin","owner":"root","parent":"","created":1467054557216,"dirs":{},"files":{"ls":{"owner":"root","created":1467054557216,"data":"(function(args){\nvar str = ' \\n';\nvar path = boss.lib.utils.calcAbsPath(args[1],args);\nvar list = boss.fs.get_contents(path);\nfor( var i in list ){\nstr += list[i] + '\\n';\n}\nboss.lib.print.log(String(str));\nboss.lib.print.log('\\n');\n})","meta":{"description":"list files in current dir"}},"mkdir":{"owner":"root","created":1467054557216,"data":"(function(args){\nif(args.length > 1){\nvar result = boss.lib.utils.splitPathFilename(args[1]);\nvar path = result.path;\nvar dirname = result.name;\n}\nif(dirname){\ntry{\nboss.fs.add_dir(path, new boss.fs.Dir(dirname, boss.fs.get_current_username(), path));\n} catch(err){\nboss.lib.print.error('error : ' + err);\n}\nboss.fs.save();\n} else if(!dirname) {\nboss.lib.print.error('error: no dir argument');\n}\n})","meta":{"description":"create new dir"}},"read":{"owner":"root","created":1467054557216,"data":"(function(args){\nif(args.length > 1){\nvar result = boss.lib.utils.splitPathFilename(args[1]);\nvar path = result.path;\nvar filename = result.name;\n}\nif(filename){\ntry{\nboss.lib.print.log(boss.fs.get_file(path, filename).data);\n} catch(err){\nboss.lib.print.error('error : ' + err);\n}\n} else if(!filename) {\nboss.lib.print.error('error: no file argument');\n}\n})","meta":{"description":"read file"}},"logout":{"owner":"root","created":1467054557216,"data":"(function(args){\nboss.lib.pop();\n})","meta":{"description":"logout"}},"edit":{"owner":"root","created":1467054557216,"data":"(function(args){if(args.length > 1){var result = boss.lib.utils.splitPathFilename(args[1]);var path = result.path;var filename = result.name;}if(filename){try{boss.fs.get_file(path,filename).data = args[2] ? args[2] : '';} catch(err){try{boss.fs.set_file(path,filename, new boss.fs.File(boss.fs.get_current_username,args[2] ? args[2] : ''));} catch(err){boss.lib.print.error('error : ' + err);}}boss.fs.save();} else if(!filename) {boss.lib.print.error('error: no file argument'); }})","meta":{"description":"edit file"}},"cd":{"owner":"root","created":1467054557216,"data":"(function(args){\nif(args[1]){\ntry{\nvar path = boss.lib.utils.calcAbsPath(args[1]);\nboss.fs.set_cwd(path);\nvar list = path.split('/');\nboss.lib.set_prompt((list.length <= 1 ? 'root' : list[list.length - 1]) + '> ');\n} catch(err){\nboss.lib.print.error('error : ' + err);\n}\n} else if(!args[1]){\nboss.lib.print.error('error: no directory argument');\n}\n})","meta":{"description":"move to dir"}},"cp":{"owner":"root","created":1467054557216,"data":"(function(args){if(args.length > 1){var result = boss.lib.utils.splitPathFilename(args[1]);var path = result.path;var filename = result.name;}if(args.length > 2){var result2 = boss.lib.utils.splitPathFilename(args[2]);var path2 = result2.path;var filename2 = result2.name;}if(filename){try{var file = boss.fs.get_file(path, filename);boss.fs.set_file(path2, filename2, file);boss.fs.save();} catch(err){boss.lib.print.error('error : ' + err);}} else if(!filename) {boss.lib.print.error('error: no file argument');}})","meta":{"description":"copy file"}},"mv":{"owner":"root","created":1467054557216,"data":"(function(args){if(args.length > 1){var result = boss.lib.utils.splitPathFilename(args[1]);var path = result.path;var filename = result.name;}if(args.length > 2){var result2 = boss.lib.utils.splitPathFilename(args[2]);var path2 = result2.path;var filename2 = result2.name;}if(filename){try{var file = boss.fs.get_file(path, filename);boss.fs.set_file(path2, filename2, file);boss.fs.remove(path, filename, 'file');boss.fs.save();} catch(err){boss.lib.print.error('error : ' + err);}} else if(!filename) {boss.lib.print.error('error: no file argument'); }})","meta":{"description":"move file"}},"rm":{"owner":"root","created":1467054557216,"data":"(function(args){\nif(args.length > 1){\nvar result = boss.lib.utils.splitPathFilename(args[1]);\nvar path = result.path;\nvar name = result.name;\n}\nif(name){\ntry{boss.fs.remove(path, name, 'file');\nboss.fs.save();\n} catch(err){\n//error('error : ' + err);\n}\ntry{boss.fs.remove(path, name, 'dir');\nboss.fs.save();\n} catch(err){\n//error('error : ' + err);\n}\n} else if(!name) {\nboss.lib.print.error('error: no file/dir argument');\n}\n})","meta":{"description":"remove file or dir"}},"js":{"owner":"root","created":1467054557216,"data":"(function(args){\nif(args.length > 1){var result = boss.lib.utils.splitPathFilename(args[1]);var path = result.path;var name = result.name;}if(name){try{boss.lib.print.log(eval(boss.fs.get_file(path,name).data));} catch(err){boss.lib.print.error('error : ' + err);}} else if(!name) {boss.lib.print.error('error: no file argument'); }})","meta":{"description":"run js file"}},"export":{"owner":"root","created":1467054557216,"data":"(function(args){\nboss.lib.print.log(JSON.stringify(boss.fs.export()));\nconsole.log(JSON.stringify(boss.fs.export()));\n})","meta":{"description":"export filesystem"}},"adduser":{"owner":"root","created":1467054557216,"data":"(function(args){\nif(args.length < 4){boss.lib.print.error('error: not enough arguments');} else if( args[2] !== 'root' && args[2] !== 'user') {boss.lib.print.error('error : invalid type ' + args[2]);} else { boss.fs.add_user(args[1], new boss.fs.User(args[1], args[2], args[3]));boss.fs.save();}})","meta":{"description":"add new user"}},"echo":{"owner":"root","created":1467054557216,"data":"(function(args){boss.lib.print.log(args[1])})","meta":{"description":"echo string to console"}},"inspect":{"owner":"root","created":1467054557216,"data":"(function(args){console.log(window)})","meta":{"description":"log window object for inspection"}},"reload":{"owner":"root","created":1467054557216,"data":"(function(args){boss.reload();})","meta":{"description":"reload BOSS"}},"test":{"owner":"root","created":1467054557216,"data":"(function(args){boss.lib.print.log('test ls');boss.cmd.ls(args);})","meta":{"description":"test fn. Calls ls"}}}},"etc":{"name":"etc","owner":"root","parent":"","created":1467054557216,"dirs":{},"files":{}},"home":{"name":"root","owner":"root","parent":"root","created":1467054557216,"dirs":{"root":{"name":"root","owner":"root","parent":"home","created":1467054557216,"dirs":{"Documents":{"name":"Documents","owner":"root","parent":"home/root","created":1467054557216,"dirs":{},"files":{}},"Downloads":{"name":"Downloads","owner":"root","parent":"home/root","created":1467054557216,"dirs":{},"files":{}},"Email":{"name":"Email","owner":"root","parent":"home/root","created":1467054557216,"dirs":{},"files":{}},"Images":{"name":"Images","owner":"root","parent":"home/root","created":1467054557216,"dirs":{},"files":{}}},"files":{}}},"files":{}},"lib":{"name":"lib","owner":"root","parent":"","created":1467054557216,"dirs":{},"files":{"print":{"owner":"root","created":1467054557216,"data":"({\n'log':function(txt){boss.term.echo(txt)},\n'error':function(txt){boss.term.echo(txt)}\n})"},"set_prompt":{"owner":"root","created":1467054557216,"data":"(function(txt){boss.term.set_prompt(txt)})"},"push":{"owner":"root","created":1467054557216,"data":"(function(intp, opts){boss.term.push(intp, opts)})"},"pop":{"owner":"root","created":1467054557216,"data":"(function(){boss.term.pop()})"},"utils":{"owner":"root","created":1467054557216,"data":"({\ncalcAbsPath : (arg, args) => {\nvar path = boss.fs.get_cwd();\nif(arg && arg.startsWith('/')){\npath = arg;\n} else if(arg && arg.startsWith('~')){\npath = '/home/' + boss.fs.get_current_username() + arg.slice(1);\n} else if(arg && arg === '..'){\nvar parts = path.split('/');\nparts = parts.slice(0, parts.length - 1);\npath = parts.join('/');\n} else if(args && args.length >= 2 && args[1]){\npath += '/' + arg;\n} else if(!args){\npath += '/' + arg;\n}\nreturn path;\n},\nsplitPathFilename : full => {\nvar path = boss.fs.get_cwd();\nvar name = full;\nvar parts = full.split('/');\nif(parts.length > 1){\nname = parts.pop();\npath = boss.lib.utils.calcAbsPath(parts.join('/'));\n}\nreturn {\npath: path,\nname: name\n}\n}\n})\n"},"Shell":{"owner":"root","created":1467054557216,"data":"(function(boss){this.exec = (command, term) => {boss.term = term;if (command !== '') {var result = command;var args = command.split(' ');if(boss.cmd[args[0]]){boss.cmd[args[0]](args);}else if(args[0] != undefined) {boss.lib.print.error('unknown command : ' + command);}}} })"},"Login":{"owner":"root","created":1467054557216,"data":"(function(boss){\nvar prompt = 'login: ';\nvar password = false;\nvar username = false;\nthis.exec = (command, term) => {\nboss.term = term;\nif(!boss.fs.get_user(command) && !password){\nboss.lib.print.error( 'unknown user');\n} else if(!password && boss.fs.get_user(command).password){\nboss.lib.set_prompt('password: ');\nusername = command;\npassword = true;\n} else if(boss.fs.get_user(username).password === command){\npassword = false;\nboss.lib.set_prompt('login: ');\nboss.fs.set_current_user(boss.fs.get_user(username));\nboss.fs.set_cwd('/home/' + username);\nboss.lib.print.log('Hello ' + username + '. Welcome to BOSS. Type `help` to see available commands.');\n//var shell = new boss.lib.Shell(boss)\nboss.lib.push(new boss.lib.Shell(boss).exec, {\n'prompt' : boss.fs.get_current_username() + '> '\n});\n} else {\nboss.lib.print.error('unknown username/password combination');\npassword = false;\nboss.lib.set_prompt(prompt);\n}\n}\n})"},"LoadFS":{"owner":"root","created":1467054557216,"data":"(function(boss){\n//choose fs to load\n})"}}},"mnt":{"name":"mnt","owner":"root","parent":"","created":1467054557216,"dirs":{},"files":{}},"tmp":{"name":"tmp","owner":"root","parent":"","created":1467054557216,"dirs":{},"files":{}},"var":{"name":"var","owner":"root","parent":"","created":1467054557216,"dirs":{},"files":{}},"sys":{"name":"sys","owner":"root","parent":"","created":1467054557216,"dirs":{},"files":{"FileSystem":{"owner":"root","created":1467054557216,"data":"(function FileSystem(r){\n//VARS\nvar root;\nvar cwd;\nvar user;\n//CLASSES\nthis.Dir = function(name, owner, parent){\nthis.name = name;\nthis.owner = owner;\nthis.parent = parent\nthis.created = Date.now();\nthis.dirs = {};\nthis.files = {};\n}\nthis.File = function(owner, data, meta){\nthis.owner = owner;\nthis.created = Date.now();\nthis.data = data;\nthis.meta = meta;\n}\nthis.User = function(username, type, password){\nthis.username = username;\nthis.type = type;\nthis.password = password;\n}\nvar get_dir = path => {\nvar parts = path.split('/');\nvar directory = root;\nfor(var i = 1; i < parts.length; i++){\nif(parts[i] !== ''){\ndirectory = directory.dirs[parts[i]];\n}\n}\nreturn directory;\n}\n//METHODS\nthis.get_cwd = () => {\nreturn cwd;\n}\nthis.set_cwd = (path) => {\nif(get_dir(path)){\ncwd = path;\n} else{\nthrow 'dir does not exist';\n}\n}\nthis.get_file = function(path, filename){\nif(get_dir(path).files[filename]){\nreturn get_dir(path).files[filename];\n} else {\nthrow 'file does not exist';\n}\n}\nthis.set_file = function(path, filename, file){\nget_dir(path).files[filename] = file;\n}\n//add directroy\nthis.add_dir = (path,dir) => { \nif(!get_dir(path).dirs[dir.name]){\nget_dir(path).dirs[dir.name] = dir;\n} else {\nthrow 'dir already exists';\n}\n}\n//remove file or dir\nthis.remove = (path, name, type) => {\nif(type === 'file' && get_dir(path).files[name]){\ndelete get_dir(path).files[name];\n} else if( type === 'dir' && get_dir(path).dirs[name] ){\ndelete get_dir(path).dirs[name];\n} else {\nthrow 'error : does not exist';\n}\n}\n//get directory and file listing\nthis.get_contents = path => {\nvar directory = get_dir(path);\nvar dirs = directory.dirs;\nvar files = directory.files;\nreturn Object.keys(dirs).concat(Object.keys(files));\n}\n//get file listing\nthis.get_files = path => {\nvar directory = get_dir(path);\nvar files = directory.files;\nreturn Object.keys(files);\n}\n//get dir listing\nthis.get_dirs = path => {\nvar directory = get_dir(path);\nvar dirs = directory.dirs;\nreturn Object.keys(dirs);\n}\n//User stuff\nthis.add_user = (key, user) => { \nroot.users[key] = user;\nvar homeDir = new this.Dir(key, key, 'home');\nhomeDir.dirs = {\n'Documents' : new this.Dir('Documents', key, 'home/' + key),\n'Downloads' : new this.Dir('Downloads', key, 'home/' + key),\n'Email' : new this.Dir('Email', key, 'home/' + key),\n'Images' : new this.Dir('Images', key, 'home/' + key)\n}\nthis.add_dir('/home', homeDir); \n}\nthis.get_user = (key) => { return root.users[key] }\nthis.set_current_user = u => { user = u } \nthis.get_current_username = () => { return user.username }\nthis.get_current_user_type = () => { return user.type }\n//return root\nthis.export = () => {\nreturn root;\n}\n//save to localStorage\nthis.save =  () => {\nlocalStorage.setItem('fs', JSON.stringify(root));\n}\nroot = r;\n})"}}}},"files":{},"users":{"root":{"username":"root","type":"root","password":"pass"}}}